...
Tinder has no friendly public API for us, so Tinder matchflow simply **pretends to be a human**. `TinderConnector` drives a real Firefox instance through Selenium: it clicks the *Messages* tab, watches for the **red dot** on unread conversations, opens a chat, reads the history and types replies.
...
### 4. Happy end 🎉
When the Analyzer detects that the girl shared a contact (phone, Insta, whatever), Tinder matchflow **ends the conversation** and sends you a **phone notification via Pushbullet** saying something like *"I planned a date with {name}"*. It also marks that person as `not_to_rise` so it won't message them again. Your turn now, human.
...
7. Login to tinder. In opened window proceed to tinder.com and login to your account. Here will appear few windows asking about permission to localisation, enebling some features, ask about buying tinder gold. Close all that windows manually as Tinder matchflow will not manage it by it's own. Check out "messages" tab also and close windows that will appear here.
8. Change name of `.env.template` file to `.env` and open it with text editor.
9. Here we need to fullfil provided fields. After "Language" provide your language (language Tinder matchflow will write in) without any parenthesis. For example, in my case it looks like: `LANGUAGE=Polish`. Also provide city you living in after "City".
10. Provede your OpenAI API key from OpenAI website.
11. Airtable:
Now we need to set up Airtable to Tinder matchflow be available remember informations about girls. Additional plus of Airtable is that memory will be common for diifferent devices if you'll use Tinder matchflow on more than one computer. Go to airtable.com and create account if you have no. Go to Yor profile icon -> "Developer Hub" -> "Personal access token" and create new token. Write some name, under the "scopes" choose all possible options. Under the scope choose "All current and future bases in all current and future workspaces". Paste it to .env file after "AIRTABLE_TOKEN=".
...
When going to date organized by Tinder matchflow, I recommend you to tell your match, that it was picked up by artificial intellegence. Beyond the fun value of the situation, it's a good practice to inform users that they are speaking with a bot.
